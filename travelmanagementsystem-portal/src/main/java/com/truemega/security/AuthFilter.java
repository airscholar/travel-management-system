...
public class AuthFilter  implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response,
		...
		try {
			if(session.getAttribute("employee") != null && requestURL.indexOf("/login.xhtml") >= 0 )
				httpServletResponse.sendRedirect(httpServletRequest.getContextPath()+ "/faces/index.xhtml");
			
			if(session.getAttribute("employee") != null || requestURL.indexOf("/login.xhtml") >= 0 || requestURL.contains("css") )
				chain.doFilter(httpServletRequest, httpServletResponse);
			else
				httpServletResponse.sendRedirect(httpServletRequest.getContextPath()+ "/faces/login.xhtml");
		} catch (Exception e) {
			loggerService.logPortalError(" filter error  ", e);
		}
